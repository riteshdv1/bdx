...
export default class KnowledgeValidationStatusAdd extends SfdxCommand {
  ...
  protected static flagsConfig = {
    name: flags.string({ char: 'n', description: 'name of the Global Picklist', required: true }),
  ...
  public async run(): Promise<any> { // tslint:disable-line:no-any
    ...
    */
    ...
    const openResult = await exec(`sfdx force:org:open -p /lightning/setup/SetupOneHome/home?0.source=alohaHeader -u ${this.flags.targetusername} -r --json`);
    const url = JSON.parse(stripcolor(openResult.stdout)).result.url;
    const page = await browser.newPage();

    //Get the instance url
    const userDetails = await exec(`sfdx force:user:display -u ${this.flags.targetusername} --json`);
    const instanceUrl = JSON.parse(stripcolor(userDetails.stdout)).result.instanceUrl;
    ...
    //Navigation starts from here
      ...
      await page.goto(instanceUrl + '/ltng/switcher?destination=classic&referrer=%2Flightning%2Fpage%2Fhome', {
        waitUntil: 'networkidle2'
      });

      await page.goto(instanceUrl + '/setup/ui/picklist_masteredit.jsp?tid=ka%23&pt=60&retURL=%2F_ui%2Fknowledge%2Fadmin%2FValidationStatusFieldsNonUddUi%3FretURL%3D%252Fui%252Fsetup%252FSetup%253Fsetupid%253DKnowledge%26setupid%3DValidationStatuses&p1=' + this.flags.name, {
        waitUntil: 'networkidle2'
      });
     ...
     .then(await page.waitForXPath(`//*[@id="bottomButtonRow"]/input[1]`).click())
      ...
      await page.waitForSelector('#phHeader > tbody > tr > td.right.vtop > div > div.navLinks > div > a.switch-to-lightning')
      await page.click('#phHeader > tbody > tr > td.right.vtop > div > div.navLinks > div > a.switch-to-lightning')
